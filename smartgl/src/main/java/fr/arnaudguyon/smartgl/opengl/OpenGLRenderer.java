...
public abstract class OpenGLRenderer implements GLSurfaceView.Renderer {
    ...
    private boolean mflip2DProj = false;

    public boolean isMflip2DProj() {
        return mflip2DProj;
    }

    public void setMflip2DProj(boolean mflip2DProj) {
        this.mflip2DProj = mflip2DProj;
    }
...
//		mTouchHelper = new TouchHelper();
    ...
    private void computeProjMatrix2D(float[] matrix2D) {
        if (!mflip2DProj) {
            ...
            Matrix.orthoM(matrix2D, 0, mWidth, 0, 0, mHeight, -1f, 1f);
        }
    }
